...
//     private static final String TEST_TAG = "jnostr_test_" + UUID.randomUUID().toString().substring(0, 8);
...
//     @Test
//     public void testSubscriptionTracking() throws Exception {
//         final CountDownLatch connectLatch = new CountDownLatch(1);
...
//         // Create a subscription with a unique tag to avoid collision with real traffic
//         NostrSubscription subscription = new NostrSubscription("test_" + TEST_TAG);
        
//         AsyncTask<NostrMessageAck> task = relay.sendMessage(subscription);
//         NostrMessageAck ack = task.get(10, TimeUnit.SECONDS);
        
//         // Since we're using a real relay, we might not always get a response 
//         // so just verify that the operation completes without exception
        
//         // Clean up by closing subscription
//         NostrSubscription.NostrSubCloseMessage closeMsg = 
//             new NostrSubscription.NostrSubCloseMessage(subscription.getSubId());
//         relay.sendMessage(closeMsg).get(10, TimeUnit.SECONDS);
