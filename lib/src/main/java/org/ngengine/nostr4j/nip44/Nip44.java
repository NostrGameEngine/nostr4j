...
public class Nip44 {
    ...
    public static byte[] getConversationKey(NostrPrivateKey privateKey, NostrPublicKey publicKey) throws Exception {
        ...
        byte[] shared = NostrUtils.getPlatform().secp256k1SharedSecret(
                privateKey._array(),
                pub);
        byte sharedX[] = Arrays.copyOfRange(shared, 1, 33);
        assert sharedX.length == CONVERSATION_KEY_SIZE;
        return NostrUtils.getPlatform().hkdf_extract(
                NIP44_V2_BYTES,
                sharedX);
    }
    ...
    private static byte[][] getMessageKeys(byte[] conversationKey, byte[] nonce) throws Exception {
        if (conversationKey == null || conversationKey.length != CONVERSATION_KEY_SIZE)
            throw new IllegalArgumentException("Conversation key must be 32 bytes");
        nonce = safeNonce(nonce);
        byte[] keys = NostrUtils.getPlatform().hkdf_expand(
                conversationKey,
                nonce,
                76 
        );   
