...
public class NostrTURN {
    ...
    public NostrTURN(
        ...
        this.inPool.connectRelay(
            new NostrRelay(localPeer.getTurnServer(),executor)
        );
        this.inSub = this.inPool.subscribe(
            new NostrFilter().author(this.remotePeer.getPubkey()).kind(this.config.getKind()).tag("d", "turn-" + this.connectionId)
        );
        ...
        this.outPool.connectRelay(
            new NostrRelay(remotePeer.getTurnServer(),executor)
        );
        this.outSub = this.outPool.subscribe(
            new NostrFilter().author(this.localPeer.getPubkey()).kind(this.config.getKind()).tag("d", "turn-" + this.connectionId)
        );
    ...
    private void onTurnEvent(SignedNostrEvent event, boolean fromRemote){
        ...
        this.localPeer.getSigner().decrypt(content, remotePeer.getPubkey()).then(decrypted->{
    ...
    private void onReceivedPacket(String data) {
        ...
        this.localPeer.getSigner().encrypt(ack, remotePeer.getPubkey()).compose(encrypted->{
            ...
            return this.localPeer.getSigner().sign(event).compose(signed->{
        ...
        this.executor.runLater(()->{
            ...
            if(entry!=null){
                ...
                for (int i = 0; i < nextPacket.chunks.size(); i++) {
                    ...
                    String encrypted = this.localPeer.getSigner().encrypt("pkt" + nextPacket.id + "," + chunkIndex + "," + nextPacket.chunks.size() + "," + chunk.data,remotePeer.getPubkey()).await();
    ...
    public AsyncTask<Void> write(ByteBuffer data){
        ...
        return platform.promisify((res,rej)->{
            ...
            // encode to b64
            String b64data = Base64.getEncoder().encodeToString(
            ...
            Packet packet = new Packet(packetId, chunks, ()->{
                res.accept(null);
            },(e)->{
                rej.accept(e);
            });
