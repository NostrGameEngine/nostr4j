...
# Inner payload envelope for deduplication

To support transparent retry/failover without duplicate delivery at the application layer, implementations MAY wrap outbound channel payloads in an inner binary envelope before handing them to WebRTC or to the TURN fallback transport.
...
PACKET_ID          uint64      // channel-local packet id, big-endian
PAYLOAD            uint8[*]    // original application bytes
```
...
* `PACKET_ID` MUST be generated by the sender from a monotonic per-channel counter.
* The same logical outbound packet SHOULD keep the same `PACKET_ID` if it is retried or if delivery switches between direct RTC and TURN fallback.
* Receivers SHOULD maintain a bounded moving window of recently seen `PACKET_ID`s per logical channel and discard duplicates.
* After duplicate filtering, receivers MUST strip the inner envelope before delivering bytes to the application/channel listener.

Backward compatibility:

* If `MAGIC` does not match, the payload MUST be treated as a legacy raw payload and delivered unchanged.
* This allows framed and unframed peers to interoperate during rollout, though duplicate suppression is only available when both sides implement the inner envelope.
